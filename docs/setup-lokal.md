...
**Tiga perintah, dan ketiganya wajib.** `--seed` hanya menjalankan
`DatabaseSeeder` (menu dan akun). Data laporan Kamila ada di seeder terpisah,
dan melewatkannya membuat seluruh dashboard tampil `Rp 0` walau aplikasinya
sendiri tidak error sama sekali.
...
| --------------------------- | -------------------------------------------------------------------- |
| `migrate --seed`            | Tidak ada menu dan tidak ada akun untuk login                        |
| `db:seed --class=LaporanSeeder` | Dashboard dan Laporan kosong: revenue `Rp 0`, semua grafik kosong |
| `storage:link`              | Gambar QRIS yang sudah diunggah tetap gagal dimuat (URL `/storage/...` 404) |

`LaporanSeeder` membaca CSV di `database/seeders/data/` dan mengisi
`laporan_transaksi` (882 baris, Juni-Juli 2026), `laporan_rfm`,
`laporan_revenue_ukuran`, dan `laporan_switch`.

> ⚠️ **`migrate:fresh` menghapus semuanya**, termasuk data laporan Kamila dan
> baris `pengaturan_toko` yang menyimpan QRIS toko. Setelah menjalankannya,
> ulangi `db:seed --class=LaporanSeeder` dan unggah ulang QRIS lewat
> Pengaturan > Info Toko. Gambar QRIS-nya sendiri tidak ikut terhapus dari
> disk, yang hilang cuma baris databasenya.
...
Ketiganya tidak memunculkan error, jadi gampang disangka bug aplikasi.

| Gejala                                                     | Penyebab                          | Perbaikan                                  |
| ---------------------------------------------------------- | --------------------------------- | ------------------------------------------ |
| Dashboard dan Laporan serba `Rp 0`, grafik kosong           | `LaporanSeeder` belum dijalankan  | `php artisan db:seed --class=LaporanSeeder` |
| Layar bayar SoyaScan bilang "Kode QRIS belum tersedia"      | Belum ada QRIS di `pengaturan_toko` | Unggah lewat Pengaturan > Info Toko       |
| QRIS sudah diunggah tapi gambarnya tetap tidak muncul       | `public/storage` belum ada         | `php artisan storage:link`                 |

Untuk memastikan datanya benar-benar ada:

```bash
php artisan tinker --execute="echo App\Models\LaporanTransaksi::count();"
# harus 882 (data Juni-Juli 2026), bukan 0
```
